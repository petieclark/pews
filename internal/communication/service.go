...
func (s *Service) setTenantContext(ctx context.Context, tenantID string) error {
	_, err := s.db.Exec(ctx, "SELECT set_config('app.current_tenant_id', $1, false)", tenantID)
	if err != nil {
...
func (s *Service) ListCampaigns(ctx context.Context, tenantID, status string) ([]Campaign, error) {
	...
	query := `
		SELECT id, tenant_id, name, template_id, channel, COALESCE(subject,''), body, COALESCE(status,'draft'),
		       scheduled_at, sent_at, COALESCE(recipient_count,0), COALESCE(opened_count,0), COALESCE(clicked_count,0),
