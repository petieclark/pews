<script>
	...
	async function submitCard() {
		...
		try {
			// Build the message field with all the additional info
			let message = '';
			if (formData.last_name) message += `Last Name: ${formData.last_name}\n`;
			if (formData.phone) message += `Phone: ${formData.phone}\n`;
			if (formData.is_first_visit) message += `First Time Visitor: Yes\n`;
			if (formData.how_hear) message += `How they heard about us: ${formData.how_hear}\n`;
			if (formData.prayer_request) message += `\nPrayer Request:\n${formData.prayer_request}`;
				...
				<form on:submit|preventDefault={submitCard} class="space-y-6">
					...
					<div class="flex items-center gap-3 p-4 bg-yellow-900 bg-opacity-20 border border-yellow-700 rounded-lg">
						<input
							type="checkbox"
							id="first-visit"
							bind:checked={formData.is_first_visit}
					...
					<div>
						<label for="how-hear" class="block text-sm font-medium text-gray-300 mb-2">
							How did you hear about us?
						</label>
						<select
							id="how-hear"
							bind:value={formData.how_hear}
					...
					<!-- Prayer Request -->
					<div>
						<label for="prayer-request" class="block text-sm font-medium text-gray-300 mb-2">
							Prayer Request or Message
						</label>
						<textarea
							id="prayer-request"
							bind:value={formData.prayer_request}
							rows="4"
							placeholder="Is there anything you'd like us to pray for? Any questions we can answer?"
							class="w-full px-4 py-3 bg-gray-900 bg-opacity-50 border border-gray-600 text-white rounded-md focus:border-teal-500 focus:ring-1 focus:ring-teal-500 focus:outline-none placeholder-gray-500 resize-none"
						></textarea>
					</div>
