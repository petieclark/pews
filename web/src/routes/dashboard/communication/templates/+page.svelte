...
<div>
	<div class="flex items-center justify-between mb-6">
		<div>
			...
			<h1 class="text-3xl font-bold mt-1" style="color: var(--text)">Message Templates</h1>
		</div>
	...
	<div class="mb-6 flex gap-2">
		{#each ['all', 'email', 'sms'] as ch}
			<button on:click={() => channelFilter = ch} class="px-4 py-2 rounded-lg font-medium transition capitalize" style="background: {channelFilter === ch ? 'var(--teal)' : 'var(--surface)'}; color: {channelFilter === ch ? 'white' : 'var(--text)'}; border: 1px solid {channelFilter === ch ? 'var(--teal)' : 'var(--border)'}">
	...
	{#if showForm}
		<div class="mb-6 rounded-lg shadow border p-6" style="background: var(--surface); border-color: var(--border)">
			<h2 class="text-lg font-semibold mb-4" style="color: var(--text)">{editId ? 'Edit' : 'New'} Template</h2>
			<div class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div>
						<label class="block text-sm font-medium mb-1" style="color: var(--text-secondary)">Name</label>
						<input bind:value={name} type="text" placeholder="Template name" class="w-full px-3 py-2 rounded-lg border" style="background: var(--bg); border-color: var(--border); color: var(--text)" />
					</div>
					<div>
						<label class="block text-sm font-medium mb-1" style="color: var(--text-secondary)">Channel</label>
						<select bind:value={channel} class="w-full px-3 py-2 rounded-lg border" style="background: var(--bg); border-color: var(--border); color: var(--text)">
				...
				<div class="grid grid-cols-2 gap-4">
					<div>
						...
						<select bind:value={category} class="w-full px-3 py-2 rounded-lg border" style="background: var(--bg); border-color: var(--border); color: var(--text)">
					...
					<div>
						...
						<input bind:value={variables} type="text" placeholder="first_name,church_name" class="w-full px-3 py-2 rounded-lg border" style="background: var(--bg); border-color: var(--border); color: var(--text)" />
					</div>
				</div>
				{#if channel === 'email'}
					<div>
						<label class="block text-sm font-medium mb-1" style="color: var(--text-secondary)">Subject</label>
						<input bind:value={subject} type="text" placeholder="Email subject" class="w-full px-3 py-2 rounded-lg border" style="background: var(--bg); border-color: var(--border); color: var(--text)" />
					</div>
				{/if}
				<div>
					<label class="block text-sm font-medium mb-1" style="color: var(--text-secondary)">Body</label>
					<textarea bind:value={body} rows="6" placeholder="Message body... Use {{variable}} for personalization" class="w-full px-3 py-2 rounded-lg border font-mono text-sm" style="background: var(--bg); border-color: var(--border); color: var(--text)"></textarea>
				</div>
				<div class="flex gap-3">
					...
					<button on:click={() => showForm = false} class="px-4 py-2 rounded-lg font-medium border" style="background: var(--surface); border-color: var(--border); color: var(--text)">Cancel</button>
				</div>
			</div>
		</div>
	{/if}
	...
	{:else if filteredTemplates.length === 0}
		<div class="rounded-lg shadow border p-12 text-center" style="background: var(--surface); border-color: var(--border)">
			...
			<h2 class="text-xl font-semibold mb-2" style="color: var(--text)">No templates yet</h2>
			...
			{#each filteredTemplates as template}
				<div class="rounded-lg shadow border p-5 hover:shadow-md transition" style="background: var(--surface); border-color: var(--border)">
					<div class="flex items-start justify-between mb-2">
						<h3 class="font-semibold" style="color: var(--text)">{template.name}</h3>
					...
					{/if}
					{#if template.subject}
						<div class="text-sm mt-2 font-medium" style="color: var(--text)">{template.subject}</div>
					{/if}
